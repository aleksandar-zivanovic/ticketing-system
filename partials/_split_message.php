...
if (!empty($child)) {
    ...
    $infoUrl     = '<a href="../user/user-view-ticket.php?ticket=' . $parentTicket[0]["id"] . '" class="font-bold text-blue-600 hover:text-blue-700" target="_blank">' . ucfirst($parentTicket[0]["title"]) . '</a>';
}
...
<div class="border-2 border-red-600 bg-yellow-100 mt-2 p-2 text-center truncate">
    ...
    <?php if (!empty($parent)) { ?>
        ...
        <ul>
            ...
            ?>
                <li>
                    <a href="../user/user-view-ticket.php?ticket=<?= $ticket["id"] ?>" class="font-bold text-blue-600 hover:text-blue-700" target="_blank"><?= ucfirst($ticket["title"]) ?></a>
                </li>
            <?php
            endforeach;
            ?>
